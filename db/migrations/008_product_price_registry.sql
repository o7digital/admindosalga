...
-- Seed the per-product register from the two Railway catalogues when both are
-- present in the same database. The storefront snapshot contains the legacy
-- USD/MXN decision; the admin listing contains the unconverted Woo amount.
DO $$
BEGIN
  IF to_regclass('public.storefront_catalog_products') IS NOT NULL THEN
    ...
    SELECT
      ...
      scp.woo_id,
      sl.external_id,
      COALESCE(sl.woo_sku, scp.sku),
      COALESCE(sl.sale_price, 0),
      CASE
        WHEN EXISTS (
          SELECT 1 FROM jsonb_array_elements(COALESCE(scp.payload->'meta_data', '[]'::JSONB)) meta
          WHERE meta->>'key' = 'dosalga_price_source_currency' AND UPPER(meta->>'value') = 'USD'
        ) THEN 'USD'
        ELSE 'MXN'
      END,
      COALESCE((
        SELECT CASE
          WHEN meta->>'value' ~ '^[0-9]+([.][0-9]+)?$' THEN (meta->>'value')::NUMERIC
        END
        FROM jsonb_array_elements(COALESCE(scp.payload->'meta_data', '[]'::JSONB)) meta
        WHERE meta->>'key' = 'dosalga_mxn_per_usd'
        LIMIT 1
      ), 17.49),
      COALESCE(scp.price, sl.sale_price, 0),
      'MXN',
      'railway-storefront-legacy-register',
      TRUE,
...
-- A price successfully published by this admin is stronger evidence than the
-- legacy import marker: it is already denominated in the listing currency.
INSERT INTO product_price_registry (
  store_code, woo_product_id, external_id, sku, raw_price, raw_currency,
  exchange_rate, final_price, final_currency, decision_source, verified, evidence
)
SELECT
  ...
  sl.currency_code,
  COALESCE(CASE
    WHEN sl.metadata->>'exchangeRate' ~ '^[0-9]+([.][0-9]+)?$'
      THEN (sl.metadata->>'exchangeRate')::NUMERIC
  END, 17.49),
  sl.sale_price,
  sl.currency_code,
