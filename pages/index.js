                  ...
                  <tbody>{pageProducts.map((product) => {
                    ...
                    return (
                      <tr key={product.id} className={publishedCurrent ? 'price-success-row' : audit.critical ? 'price-alert-row' : ''}>
                        ...
                        <td><span className={`currency-pill ${product.currencyMismatch ? 'currency-mismatch' : ''}`}>{product.saleCurrency}</span><small className="currency-source">WP import · {product.importedCurrency || product.sourceCurrency || product.saleCurrency}</small>{product.currencyMismatch && <small className="danger-text">Expected {product.expectedStoreCurrency}</small>}</td>
