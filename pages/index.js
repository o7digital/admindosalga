...
const blankProduct = {
  ...
  cjCostCurrency: 'USD',
  cjOriginalCostUsd: 0,
  cjOriginalCostCurrency: 'USD',
  cjOriginalCostAt: null,
  salePrice: 0,
  saleCurrency: 'USD',
  sourcePrice: 0,
  sourceCurrency: 'USD',
...
function normalizeProduct(product) {
  ...
  return {
    ...
    cjOriginalCostUsd: Number(product.cjOriginalCostUsd) || 0,
    ...
    importedCurrency: product.importedCurrency || product.sourceCurrency || product.saleCurrency || (siteId === 'dosalga-usa' ? 'USD' : 'MXN'),
...
export default function ProductControl() {
  ...
  const orderSummary = useMemo(() => ({
    revenueMx: visibleOrders.filter((order) => order.currency === 'MXN' && order.recognizedRevenue).reduce((sum, order) => sum + order.total, 0),
    revenueUs: visibleOrders.filter((order) => order.currency === 'USD' && order.recognizedRevenue).reduce((sum, order) => sum + order.total, 0),
    profitMx: visibleOrders.filter((order) => order.currency === 'MXN' && order.recognizedRevenue && order.estimatedProfit !== null).reduce((sum, order) => sum + order.estimatedProfit, 0),
    profitUs: visibleOrders.filter((order) => order.currency === 'USD' && order.recognizedRevenue && order.estimatedProfit !== null).reduce((sum, order) => sum + order.estimatedProfit, 0),
            ...
            {activeView === 'Orders' && (
              <section className="catalog-card compact-card">
                ...
                <div className="table-wrap"><table><thead><tr><th>Woo order</th><th>Store</th><th>Customer</th><th>Status</th><th>Items / CJ link</th><th>Total</th><th>CJ estimated cost</th><th>Profit / loss</th><th>Date</th></tr></thead><tbody>
                  {visibleOrders.map((order) => <tr key={order.id} className={(order.estimatedProfit !== null && order.estimatedProfit < 0) || order.unmatchedItems ? 'price-alert-row' : ''}><td><strong>#{order.wooOrderId}</strong></td><td><span className={`market-badge ${order.market === 'US' ? 'usa' : 'mexico'}`}><span className={`flag ${order.market === 'US' ? 'us' : 'mx'}`} />{order.market}</span></td><td>{order.customer}</td><td><strong>{order.status}</strong><small>{order.recognizedRevenue ? 'Paid/active revenue' : 'Not counted in revenue'}</small>{order.refund > 0 && <small>Refund {formatCurrency(order.refund, order.currency)}</small>}</td><td>{order.items.map((item) => <small key={item.id} className={item.linked ? 'included' : 'danger-text'}>{item.quantity}× {item.sku || item.name} · {item.linked ? `CJ ${item.cjSku}` : 'SKU not linked'}</small>)}</td><td><strong>{formatCurrency(order.total, order.currency)}</strong><small>Tax {formatCurrency(order.tax, order.currency)} · Shipping {formatCurrency(order.shipping, order.currency)}</small></td><td><strong>{order.unmatchedItems ? 'Incomplete' : formatCurrency(order.estimatedCost, order.currency)}</strong></td><td><strong className={order.estimatedProfit !== null && order.estimatedProfit < 0 ? 'danger-text' : 'included'}>{order.estimatedProfit === null ? 'Unknown' : formatCurrency(order.estimatedProfit, order.currency)}</strong><small>{order.unmatchedItems ? 'Link the SKU to calculate' : 'Estimated'}</small></td><td>{order.date ? new Date(order.date).toLocaleString() : '—'}</td></tr>)}
