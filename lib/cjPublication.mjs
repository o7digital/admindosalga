...
export function publicationVariants(detail, identity, proposal) {
  ...
  if (proposal.currency !== identity.currency || detail.platformProductPricesCurrency !== identity.currency) throw new Error(`CJ currency: ${detail.platformProductPricesCurrency || 'not provided'}; dashboard: ${proposal.currency}. Verify the source currency before publishing.`);
  ...
  return detail.variants.map(v => {
    if (String(v.platformProductId) !== identity.productId || !v.platformVariantId || seen.has(String(v.platformVariantId))
      || !v.platformVariantSku || !v.platformVariantTitle || !v.platformVariantImage || v.priceCurrency !== identity.currency) {
