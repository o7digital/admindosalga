...
export const saveCompetitorOffer = async ({ externalId, competitorName, price, shippingCost = 0, url = '' }) => withTransaction(async (client) => {
  ...
  const result = await client.query(`
    INSERT INTO competitor_offers (
      ...
      currency_code, shipping_cost, match_confidence, captured_at
    ) VALUES ($1, $2, $3, NULLIF($4, ''), $5, $6, $7, 1, NOW())
    RETURNING price, shipping_cost, currency_code, competitor_url, captured_at
  `, [row.product_id, row.store_id, competitor === 'temu' ? 'Temu' : 'Amazon', url, price, row.currency_code, shippingCost]);
