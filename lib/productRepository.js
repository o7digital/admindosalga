...
const upsertOne = async (client, product) => {
  ...
  const listingResult = await client.query(`
    ...
    ON CONFLICT (external_id) DO UPDATE SET
      ...
      metadata = (EXCLUDED.metadata - 'cjPriceProposal' - 'cjPricePublication' - 'wooPricePublication' - 'wooFrozen' - 'wooFreeze') || CASE
...
export async function finishCjPublication(externalId, operation, publication) {
  await query(`UPDATE store_listings SET metadata = jsonb_set(metadata, '{cjPricePublication}', $3::jsonb)
    WHERE external_id = $1 AND metadata->'cjPricePublication'->>'operation' = $2`,
  [externalId, operation, JSON.stringify({ ...publication, operation })]);
}
...
export async function beginWooPricePublication(externalId, savedAt) {
  const result = await query(`
    ...
    WHERE external_id = $1 AND active = TRUE
      AND metadata->'cjPriceProposal'->>'savedAt' = $2
      AND COALESCE(metadata->'wooPricePublication'->>'state', '') NOT IN ('sending', 'unknown')
...
export async function finishWooPricePublication(externalId, operation, publication) {
  ...
  [externalId, operation, JSON.stringify({ ...publication, operation }), publication.price]);
}
