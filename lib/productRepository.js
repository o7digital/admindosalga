...
export const listProducts = async () => {
  ...
  const result = await query(`
    SELECT
      ...
      cj.original_product_cost AS cj_original_product_cost,
      cj.original_currency_code AS cj_original_currency_code,
      cj.original_captured_at AS cj_original_captured_at,
    ...
    LEFT JOIN LATERAL (
      SELECT mappings.cj_pid,
             ...
             original_snapshot.product_cost AS original_product_cost,
             original_snapshot.currency_code AS original_currency_code,
             original_snapshot.captured_at AS original_captured_at
      ...
      LEFT JOIN LATERAL (
        SELECT product_cost, currency_code, captured_at
        FROM cj_cost_snapshots
        WHERE cj_mapping_id = mappings.id
          AND product_cost > 0
        ORDER BY captured_at ASC, id ASC
        LIMIT 1
      ) original_snapshot ON TRUE
  ...
  return result.rows.map((row) => {
    const product = normalizeWooPrice({
      ...
      originalProductCost: row.cj_original_product_cost,
      originalCurrency: row.cj_original_currency_code,
      originalCapturedAt: row.cj_original_captured_at,
    }),
    id: row.external_id,
    ...
    return applyPriceRegistry(product, row.registry_raw_currency ? {
...
export const listPriceRegistry = async () => {
  await ensurePriceRegistrySchema();
  const result = await query(`
    SELECT store_code, woo_product_id, external_id, sku, raw_price, raw_currency,
           exchange_rate, final_price, final_currency, decision_source, verified,
           updated_at
    FROM product_price_registry
    ORDER BY store_code, woo_product_id
  `);
  return result.rows.map((row) => ({
