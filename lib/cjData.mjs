...
export const applyCjCostUpdate = (product, cjProduct, now = new Date().toISOString()) => {
  ...
  return {
    product: {
      ...
      cjCostUsd: cjCost,
      cjCostCurrency: 'USD',
      cjOriginalCostUsd: parseCjPrice(product?.cjOriginalCostUsd) || previousCost || cjCost,
      cjOriginalCostCurrency: product?.cjOriginalCostCurrency || product?.cjCostCurrency || 'USD',
      cjOriginalCostAt: product?.cjOriginalCostAt || product?.lastCjSyncAt || now,
...
export const overlayCjSnapshot = (metadata = {}, snapshot = {}) => {
  ...
  const originalCost = parseCjPrice(snapshot.originalProductCost);
  if (originalCost > 0) {
    merged.cjOriginalCostUsd = originalCost;
    merged.cjOriginalCostCurrency = snapshot.originalCurrency || snapshot.currency || 'USD';
    if (hasValue(snapshot.originalCapturedAt)) merged.cjOriginalCostAt = snapshot.originalCapturedAt;
  }
